            ...
            ->columns([
                ...
                TextColumn::make('morning_start')
                    ...
                    ->state(function (Schedule $record): string {
                        return mb_substr((string) $record->morning_start, 0, 5).' - '.mb_substr((string) $record->morning_end, 0, 5);
                    }),
                TextColumn::make('noon_start')
                    ->label('Après-midi')
                    ->state(function (Schedule $record): string {
                        return mb_substr((string) $record->noon_start, 0, 5).' - '.mb_substr((string) $record->noon_end, 0, 5);
                    }),
            ])
            ->headerActions([
                CreateAction::make()
