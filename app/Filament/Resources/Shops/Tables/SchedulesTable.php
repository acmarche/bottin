        ...
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('morning_start')
                    ->label('Matin')
                    ->state(fn (Schedule $record): string => $record->morning_start && $record->morning_end
                        ? $record->morning_start.' - '.$record->morning_end
                        : '-'),
                TextColumn::make('noon_start')
                    ->label('Après-midi')
                    ->state(fn (Schedule $record): string => $record->noon_start && $record->noon_end
                        ? $record->noon_start.' - '.$record->noon_end
                        : '-'),
            ])
            ->headerActions([
                CreateAction::make()
