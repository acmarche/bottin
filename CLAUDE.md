...
Bottin is an employee directory/user management application built on the **Larament** starter kit (Laravel 12 + Filament 5). It integrates with LDAP (Active Directory) for authentication against `ldap.marche.be`.
...
php artisan test --compact tests/Feature/File.php  # Run specific file
...
composer review        # Run all: pint → rector → phpstan → pest
```
...
### Creating Files
```bash
php artisan make:model ModelName --no-interaction    # Always include factory + seeder
php artisan make:test --pest TestName --no-interaction
php artisan make:test --pest --unit TestName --no-interaction
```

## Architecture

### Filament Resource Organization
Resources use a nested folder structure with separate classes for forms and tables:
```
app/Filament/Resources/Users/
├── UserResource.php           # Resource definition, navigation, global search
├── Pages/
│   ├── CreateUser.php
│   ├── EditUser.php
│   └── ListUsers.php
...
└── Tables/
    └── UsersTable.php          # Table column/filter/action definitions
```
When creating new resources, follow this same pattern of extracting forms and tables into dedicated classes.

### LDAP Authentication
- `app/Auth/LdapAuthService.php` — Checks credentials against LDAP; bypasses in local/testing environments
- `app/Ldap/User.php` — LDAP user model
- `app/Ldap/UserHandler.php` — Syncs LDAP users to local database
- `app/Filament/Pages/Auth/Login.php` — Custom login page (auto-fills credentials in local env)

### Key Configuration
- `config/app.php` — Contains default user credentials for local development
- `config/essentials.php` — Nuno Maduro's Essentials (auto eager-loading, aggressive prefetching)
- `bootstrap/app.php` — Middleware, exceptions, routing (Laravel 12 style)
- `bootstrap/providers.php` — Service providers
- `app/Helpers.php` — Autoloaded helper functions

### Testing
- Tests auto-authenticate via `TestCase::setUp()` using `User::factory()->create()` with default user config
- Uses `RefreshDatabase` trait and SQLite in-memory for all test groups (Feature, Unit, Browser)
- Filament tests use `livewire()` helper for component testing

### Strict PHP Conventions (enforced by Pint)
- `declare(strict_types=1)` on all files
- `final` classes by default
- Strict comparison (`===`) only
- Global namespace imports for classes, functions, and constants
- Ordered class elements: traits → constants → properties → constructor → methods
- `protected_to_private` — use private unless inheritance is needed
- `DateTimeImmutable` preferred over `DateTime`
- `mb_str_functions` — use multibyte string functions

### Database
- MariaDB in production, SQLite in-memory for tests
